
CompactRaidDB = {
	["showSolo"] = 1,
	["showtooltip"] = 1,
	["healthColor"] = "0.0,1.0,0.0",
	["showParty"] = 1,
	["modules"] = {
		["RaidDebuff"] = {
			["scale"] = 100,
			["userLevels"] = {
				["7,768,203102"] = 0,
				["7,768,203124"] = 0,
			},
			["selTier"] = 7,
			["customDebuffs"] = {
			},
			["xoffset"] = 0,
			["selBoss"] = 1725,
			["selInstance"] = 786,
			["yoffset"] = 0,
		},
		["Artwork"] = {
			["statusbar"] = "Interface\\BUTTONS\\WHITE8X8.BLP",
			["background"] = "Interface\\DialogFrame\\UI-DialogBox-Background",
			["border"] = "Interface\\Tooltips\\UI-Tooltip-Border",
			["font"] = "Fonts\\ARKai_T.ttf",
		},
	},
	["powerColor"] = "0.0,0.0,1.0",
	["groupSwapNoNotify"] = 1,
	["showToolboxes"] = 1,
	["outrangeAlpha"] = 40,
	["showRoleIcon"] = 1,
	["showDebuffs"] = 1,
	["nameWidthLimit"] = 75,
	["showDispels"] = 1,
	["unitBkColor"] = "0.0,0.0,0.0",
	["showbarbkgnd"] = 1,
	["height"] = 36,
	["showPrivIcons"] = 1,
	["nameYOffset"] = 0,
	["tooltipPosition"] = 1,
	["scale"] = 100,
	["showBuffs"] = 1,
	["spacing"] = 1,
	["showRaidIcon"] = 1,
	["containerBorderSize"] = 12,
	["powerBarHeight"] = 4,
	["raidFilter"] = "CLASS",
	["version"] = 5.09,
	["profiles"] = {
		["喵言术嗷呜 - 燃烧之刃"] = {
			["modules"] = {
				["Artwork"] = {
					["disabled"] = 1,
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["RaidDebuff"] = {
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
		},
		["泠凊腤 - 拉文凯斯"] = {
			["modules"] = {
				["Artwork"] = {
					["disabled"] = 1,
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["RaidDebuff"] = {
				},
			},
			["version"] = 4.22,
			["showPartyPets"] = 1,
		},
		["尊严德 - 燃烧之刃"] = {
			["modules"] = {
				["Artwork"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["RIGHT"] = "[selfcast]#1#[aura]#复苏之雾#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[r1]#0.0#[r2]#1.0#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
						["BOTTOMRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[threshold3]#3#[r1]#0.0#[r2]#1.0#",
						["BOTTOM"] = "[selfcast]#1#[aura]#精华之泉#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[threshold3]#3#[r1]#0.0#[r2]#1.0#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
					},
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["RaidDebuff"] = {
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
		},
		["Testlze - 燃烧之刃"] = {
			["modules"] = {
				["Artwork"] = {
					["disabled"] = 1,
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["RaidDebuff"] = {
				},
			},
			["version"] = 4.22,
			["showPartyPets"] = 1,
		},
		["立二拆三 - 燃烧之刃"] = {
			["modules"] = {
				["RaidDebuff"] = {
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["Artwork"] = {
					["disabled"] = 1,
				},
			},
			["version"] = 4.25,
			["showPartyPets"] = 1,
		},
		["泠凊之腤 - 白银之手"] = {
			["modules"] = {
				["Artwork"] = {
					["disabled"] = 1,
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["RaidDebuff"] = {
				},
			},
			["version"] = 5.06,
			["showPartyPets"] = 1,
		},
		["备前长船 - 阿克蒙德"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent2"] = {
					},
					["talent1"] = {
					},
				},
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:暗影步",
						["ctrl-1"] = "buildin:嫁祸诀窍",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:暗影步",
						["ctrl-1"] = "buildin:嫁祸诀窍",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 4.31,
			["showPartyPets"] = 1,
		},
		["备前长光 - 白银之手"] = {
			["modules"] = {
				["Artwork"] = {
					["disabled"] = 1,
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["RaidDebuff"] = {
				},
			},
			["version"] = 4.33,
			["showPartyPets"] = 1,
		},
		["Pantyazure - 燃烧之刃"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent3"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
				},
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 4.33,
			["showPartyPets"] = 1,
		},
		["萌娘百科 - 燃烧之刃"] = {
			["modules"] = {
				["Artwork"] = {
					["disabled"] = 1,
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["RaidDebuff"] = {
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
		},
		["喵喵了个咪肉 - 燃烧之刃"] = {
			["modules"] = {
				["RaidDebuff"] = {
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["Artwork"] = {
					["disabled"] = 1,
				},
			},
			["version"] = 4.22,
			["showPartyPets"] = 1,
		},
		["教授猫 - 白银之手"] = {
			["modules"] = {
				["RaidDebuff"] = {
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["Artwork"] = {
					["disabled"] = 1,
				},
			},
			["version"] = 5.06,
			["showPartyPets"] = 1,
		},
		["喵喵围炉斯 - 燃烧之刃"] = {
			["modules"] = {
				["RaidDebuff"] = {
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["Artwork"] = {
					["disabled"] = 1,
				},
			},
			["version"] = 4.22,
			["showPartyPets"] = 1,
		},
		["暗言术喵 - 燃烧之刃"] = {
			["modules"] = {
				["RaidDebuff"] = {
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#真言术：盾#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#恢复#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#愈合祷言#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#真言术：盾#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#愈合祷言#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#恢复#[style]#2#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#恢复#[style]#2#",
						["RIGHT"] = "[selfcast]#1#[aura]#救赎#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[r1]#0.0#[r2]#1.0#",
						["BOTTOMRIGHT"] = "[selfcast]#1#[aura]#救赎#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[style]#2#[r1]#0.0#[r2]#1.0#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#愈合祷言#",
						["TOPRIGHT"] = "[selfcast]#1#[ignoreVehicle]#1#[aura]#救赎#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[style]#3#[r1]#0.0#[r2]#1.0#",
					},
				},
				["Artwork"] = {
					["disabled"] = 1,
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["自杀贼 - 燃烧之刃"] = {
			["modules"] = {
				["RaidDebuff"] = {
				},
				["ClickSets"] = {
					["disabled"] = 1,
				},
				["CornerIndicators"] = {
					["disabled"] = 1,
				},
				["Artwork"] = {
					["disabled"] = 1,
				},
			},
			["version"] = 4.33,
			["showPartyPets"] = 1,
		},
	},
	["width"] = 64,
	["showDirectionArrow"] = 1,
	["nameHeight"] = 12,
	["nameColor"] = "1.0,1.0,1.0",
	["nameXOffset"] = 0,
	["containerAlpha"] = 75,
	["keepgroupstogether"] = 1,
	["healthtextmode"] = 0,
}
